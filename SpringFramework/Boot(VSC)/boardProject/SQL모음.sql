...
ALTER USER spring_bdh IDENTIFIED BY spring1234;
...
-- COUNT(*) : 조회된 모든 행의 개수 (그룹 함수 == 결과가 1행)
-- OVER() : 각 행의 대한 계산을 수행하는 함수(오라클에서는 이런 함수를 윈도우 함수라고함)
	
-- COUNT(*) OVER() : 각 행마다 현재 SQL에서 조회된 모든 행의 개수를 추가
-- ROW_NUMBER() OVER(ORDER BY BOARD_NO) : 각 행마다 BOARD_NO를 오름 차순으로 추가 
	
-- CEIL(소수) : 올림 처리
-- CP : 현재 게시글의 페이지 번호
SELECT RNUM, LIST_COUNT, CEIL((LIST_COUNT - RNUM + 1) / 10) CP
FROM (
	SELECT 
		ROW_NUMBER() OVER(ORDER BY BOARD_NO ASC) RNUM, 
		BOARD_NO,
...
SELECT ROWNUM, BOARD_NO, CEIL(ROWNUM / 10)
		FROM "BOARD"
		WHERE BOARD_CODE = 1
--		AND BOARD_NO = 5350
		AND BOARD_DEL_FL = 'N';
...
FROM(
	SELECT 
		ROW_NUMBER() OVER(ORDER BY BOARD_NO DESC) RNUM, BOARD_NO
	FROM "BOARD"
	WHERE BOARD_CODE = 1
	AND BOARD_DEL_FL = 'N'
)
...
/* 알림 받는 회원 번호 + 읽지 않은 알림 수 조회*/
SELECT 
	RECEIVE_MEMBER_NO "recevieMemberNo",
	(SELECT COUNT(*) 
