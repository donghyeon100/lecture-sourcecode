...
public class BoardServiceImpl implements BoardService{
	...
	public Map<String, Object> selectBoardList(int boardCode, int cp) {
		...
		// boardList.stream()
		// 	.filter(board -> !board.getImagesList().isEmpty() && board.getImagesList().get(0).getImgOrder() == 0)
		// 	.forEach(t -> t.setThumbnail(t.getImagesList().get(0).getImagePath()));
		boardList.forEach(board -> {
			if(!board.getImageList().isEmpty()) {
				if(board.getImageList().get(0).getImgOrder() == 0)	{

					BoardImg img = board.getImageList().get(0);
					board.setThumbnail(img.getImgPath() + img.getImgRename());
				}
			}
		});
