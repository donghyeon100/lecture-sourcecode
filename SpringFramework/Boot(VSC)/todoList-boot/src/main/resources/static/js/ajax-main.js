...
const popuptodoDetail = document.querySelector("#popuptodoDetail");
...
// todoCompleteCount 값 비동기 통신으로 얻어와서 화면 출력
function getCompleteCount(){
  ...
  // 두 번째 then의 result
  // - 첫 번째 then에서 text로 변환된 응답 데이터(todoCompleteCount값)

  fetch("/ajax/todoCompleteCount")
  .then( response => {return response.text()} )
  //  ==  .then( response => response.text()} )
  .then(result => {

    // #todoCompleteCount 요소에 내용으로 result값 출력
    todoCompleteCount.innerText = result;
  })
}
...
const selectTodo = (url) => {
  ...
  .then(todo => {
    ...
    popuptodoDetail.innerText = todo.todoDetail;
...
const selectTodoList = () => {
  ...
  .then(result => { // result == 첫 번째 then에서 반환된 결과값
    ...
    for(let todo of todoList){ // 향상된 for문
      ...
      const arr = ['todoNo', 'todoTitle', 'todoComplete', 'regDate'];
...
changeComplete.addEventListener('click', e => {
  ...
  const todoComplete = popupComplete.innerText === 'Y' ? 1 : 0; 

  // SQL 수행에 필요한 값을 객체로 묶음
  const obj = {"todoNo" : todoNo,  "todoComplete" : todoComplete};
  ...
  .then( result => {
    ...
    if(result > 0){
      ...
      popupComplete.innerText = todoComplete === 1 ? 'Y' : 'N'; 

      //getCompleteCount();
      // 서버 부하를 줄이기 위해 완료된 Todo 개수 +-1

      const count = Number(todoCompleteCount.innerText);

      if(todoComplete === 'Y') todoCompleteCount.innerText = count + 1;
      else                 todoCompleteCount.innerText = count - 1;
...
updateView.addEventListener("click", () => {
  ...
  updateContent.value 
    = popuptodoDetail.innerHTML.replaceAll("<br>", "\n");
...
updateBtn.addEventListener("click", e => {
  ...
  const obj = {
    ...
    "todoDetail" : updateContent.value
  };
